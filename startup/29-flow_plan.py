...
def reset_pumps2(pump_list, clear=True, update=".2 second"):
    for pump in pump_list:
        pump.initialize_pump(clear=clear, update=update)
        yield from bps.mv(
            pump.infuse_rate_unit,
            "ul/min",
            pump.infuse_rate,
            100,
            pump.withdraw_rate_unit,
            "ul/min",
            pump.withdraw_rate,
            100,
            pump.target_vol_unit,
            "ml",
            pump.target_vol,
            20,
        )
...
def show_pump_status(
    syringe_list, pump_list, precursor_list, syringe_mater_list, wait=False
):
    for input_size, pump, precursor, material in zip(
        syringe_list, pump_list, precursor_list, syringe_mater_list
    ):
...
def set_group_infuse(
    ...
    for i, j, k, l, m in zip(
        pump_list, target_vol_list, rate_list, syringe_list, syringe_mater_list
    ):
        vol = float(j.split(" ")[0])
        vol_unit = j.split(" ")[1]
        rate = float(k.split(" ")[0])
        rate_unit = k.split(" ")[1]
        yield from i.set_infuse(
            l,
            target_vol=vol,
            target_unit=vol_unit,
            infuse_rate=rate,
...
def set_group_withdraw(
    ...
    for i, j, k, l, m in zip(
        ...
        rate = float(k.split(" ")[0])
        rate_unit = k.split(" ")[1]
        yield from i.set_withdraw(
            l,
            target_vol=vol,
            target_unit=vol_unit,
            withdraw_rate=rate,
            withdraw_unit=rate_unit,
            syringe_material=m,
        )
...
def _readable_time(unix_time):
    ...
    print(
        f"{dt.year}{dt.month:02d}{dt.day:02d},{dt.hour:02d}{dt.minute:02d}{dt.second:02d}"
    )
...
def insitu_test(
    ...
    for i in range(abs_repeat):
        yield from qepro.take_uvvis_save_csv(
            sample_type=sample,
            csv_path=csv_path,
...
def take_ref_bkg_q(
    ...
    yield from qepro.setup_collection2(
        integration_time=integration_time,
        num_spectra_to_average=num_spectra_to_average,
        buffer=buffer,
        spectrum_type="Absorbtion",
        correction_type="Reference",
        electric_dark_correction=True,
    ...
    if csv_path != None:
        ...
        qepro.export_from_scan(
            uid,
            csv_path,
            sample_type=f"Dark_{integration_time}ms",
            data_agent=data_agent,
        )
    ...
    if csv_path != None:
        ...
        qepro.export_from_scan(
            uid,
            csv_path,
            sample_type=f"{ref_name}_{integration_time}ms",
            data_agent=data_agent,
        )
...
def take_a_uvvis_csv_q(
    ...
    if pump_list != None and precursor_list != None:
        _md = {
            ...
            "uvvis": [
                spectrum_type,
                correction_type,
                qepro.integration_time.get(),
                qepro.num_spectra.get(),
                qepro.buff_capacity.get(),
            ],
            "mixer": mixer,
    ...
    if spectrum_type == "Absorbtion":
        if (
            ...
            and qepro.correction.get() == correction_type
            and qepro.spectrum_type.get() == spectrum_type
        ):
    ...
    if csv_path != None or plot == True:
        yield from bps.sleep(2)
        qepro.export_from_scan(
            uid, csv_path, sample_type, plot=plot, data_agent=data_agent
        )


def take_a_uvvis_csv_q3(
    sample_type="test",
    plot=False,
    csv_path=None,
    data_agent="tiled",
...
def l_unit_converter(l0="m", l1="m"):
    ...
    l_frame = pd.DataFrame(
        data={
            "mm": np.array([1, 10, 1000]),
            "cm": np.array([0.1, 1, 100]),
            "m": np.array([0.001, 0.01, 1]),
        },
        index=l_unit,
    )
...
def wait_equilibrium(pump_list, mixer, ratio=1, tubing_ID_mm=1.016):
    ...
    for i in range(len(infuse_rates)):
        ...
        unit_const = vol_unit_converter(v0=rate_unit[:2], v1="ul") / t_unit_converter(
            t0=rate_unit[3:], t1="min"
        )
    ...
    print(
        f"Wait for {ratio} times of resident time, in total of {res_time_sec * ratio:.2f} seconds."
    )
...
def wait_equilibrium2(mixer_pump_list, ratio=1, tubing_ID_mm=1.016):
    ...
    for mixer_pump in mixer_pump_list:
        ...
        for i in range(len(infuse_rates)):
            rate = infuse_rates[i]
            rate_unit = infuse_rate_unit[i]
            unit_const = vol_unit_converter(
                v0=rate_unit[:2], v1="ul"
            ) / t_unit_converter(t0=rate_unit[3:], t1="min")
