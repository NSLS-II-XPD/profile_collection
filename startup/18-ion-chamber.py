...
class IonChamber(Device):
    ...
    initiate = Cpt(EpicsSignal, "Init", kind=Kind.omitted)
    stop_signal = Cpt(EpicsSignal, "Abort", kind=Kind.omitted)
    save_signal = Cpt(EpicsSignal, "Save", kind=Kind.omitted)
    ...
    def trigger(self):
        ...
        def cb(value, old_value, **kwargs):
            ...
            if int(value) <= int(self.max_counts.get()):
